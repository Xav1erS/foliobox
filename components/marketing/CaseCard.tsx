  ...
  return (
    <div className="flex h-full">
      ...
      <div className="flex-1 p-3">
        ...
        <div className="h-14 rounded bg-white/[0.03] border border-white/[0.06] p-2">
  ...
  return (
    <div className="flex h-full items-center justify-center gap-3 px-4">
      {[
        ...
        <div key={i} className="h-[120px] w-[68px] rounded-xl border border-white/10 bg-white/[0.04] p-2" style={{ transform: i === 1 ? "translateY(10px)" : undefined }}>
          <div className="mb-2 flex items-center gap-1">
            <div className="h-1.5 w-1.5 rounded-full bg-white/20" />
            <div className="h-1.5 w-8 rounded bg-white/10" />
          </div>
          <div className={`mb-2 h-8 w-full rounded ${s.accent}`} />
          <div className="space-y-1.5">
...
export function CaseCard({
  ...
  return (
    <div className="group flex flex-col rounded-2xl border border-white/8 bg-white/[0.02] transition-all duration-200 hover:border-white/15 hover:bg-white/[0.04]">
      {/* Mock preview image */}
      <div className="relative overflow-hidden rounded-t-2xl border-b border-white/[0.06] bg-white/[0.02]" style={{ height: 148 }}>
        <MockComponent />
        {/* Demo badge */}
        <div className="absolute left-3 top-3 rounded-full border border-white/10 bg-black/60 px-2.5 py-0.5 text-[10px] text-white/40 backdrop-blur-sm">
          演示案例
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mb-4 flex items-center gap-2">
          <span className="rounded-md bg-white/5 px-2.5 py-1 text-xs tabular-nums text-white/30 line-through">
            {scoreBefore}
          </span>
        ...
        {/* Description */}
        <p className="mt-3 flex-1 text-sm leading-relaxed text-white/45">{description}</p>
        ...
        <div className="mt-4 flex flex-wrap gap-1.5">
          {tags.map((tag) => (
            <span
              key={tag}
              className="rounded-full border border-white/8 px-2.5 py-0.5 text-xs text-white/35"
            >
              {tag}
            </span>
          ))}
