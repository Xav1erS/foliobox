        ...
        return (
          <section key={section.heading} className="space-y-6">
            ...
            {section.bullets?.length ? (
              <ul className="marketing-panel space-y-3 border border-white/8 bg-white/2 px-6 py-6">
                {section.bullets.map((bullet) => (
                  <li key={bullet} className="flex items-start gap-3 text-base leading-7 text-white/68">
                    <span className="marketing-pill mt-3 h-1.5 w-1.5 shrink-0 bg-emerald-400" />
                    <span>{bullet}</span>
                  </li>
                ))}
              </ul>
